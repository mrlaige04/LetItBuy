...
namespace Shop.Services
{
    public class PhotoService
    {
        ...
        public async Task<ServicesResultModel> SetUserProfileImage(IFormFile photo, string userId)
        {
            ...
            {
                return new ServicesResultModel { ResultCode = ResultCodes.Successed, Errors = result.Errors.Select(x => x.Description).ToList() };
            }
        }
        ...
        public async Task<ServicesResultModel> DeleteUserImage(string userId)
        {
            ...
            if (filePath != null)
            {
                ...
                {
                    return new ServicesResultModel { ResultCode = ResultCodes.Failed, Errors = deleteImage_Result.Errors.Select(x => x.Description).ToList() };
                }
            }
            else
            {
                return new ServicesResultModel { ResultCode = ResultCodes.Failed, Errors = new List<string> { "No file found" } };
            }
        }
